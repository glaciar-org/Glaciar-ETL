...
FILE_OUTPUT="STAGE_02_MongoDB/STEP-03/MGSET_03_2015_2017.csv"

TITULOS=${DATE_TIME}.date\(2006-01-02:15:04\),${DATE}.string\(\),${TIME}.string\(\),${WATERQ_Temp}.string\(\),${WATERQ_pH}.string\(\),${WATERQ_OD}.string\(\),${WATERQ_OD_pc}.string\(\),${DATASET}.string\(\)

if [ ${MONGODB_MLAB_UPSALA_INSERT} == "true" ]; then
        cvs2MongoDB  ${FILE_OUTPUT}  "MGSET_03_2015_2017" ${TITULOS} 
fi
